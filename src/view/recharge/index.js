...
import "./index.scss";
import { List, Search, NavBar, Input, NumberKeyboard, Button, Toast } from "antd-mobile";
...
class Recharge extends Component {
  ...
  async onGotoRecharge(){
    ...
    if(!rechargePrice) return Toast.show({
      // icon: "-",
      duration: 1000,
      content: '请输入充值金额',
    });
  ...
  render() {
    ...
    let { isVisible,rechargePrice,fontPriceIndex,fontPrice,submiting } = this.state;
    return <div className="recharge-container">
      <NavBar
        style={{
          "--height": "0.44rem",
          color: "#000",
      ...
      >
        <List.Item title='充值金额'>
          <div className="font-price">
            {fontPrice[fontPriceIndex]?<span>| <span className="font">{fontPrice[fontPriceIndex]}</span></span>:null}
          </div>
          <div className="recharge-price" onClick={()=>{
            this.setState({
              isVisible:true
            })
          }}>
            {!isVisible && !rechargePrice?'请输入金额':<span className="price">{rechargePrice}</span>}
            {isVisible?<div className="focus-bar"></div>:null}

            <CloseCircleFill color='var(--adm-color-primary)' className="input-close" fontSize={20} onClick={()=>{
              this.setState({
                rechargePrice:'',
                fontPriceIndex:0
              })
      ...
      <Button block color='primary' disabled={submiting} size='large' className="recharge-button" onClick={()=>{
        this.onGotoRecharge();
      }}>充值</Button>
      ...
      <NumberKeyboard
        ...
        onInput={(val)=>{
          ...
          if((val=='.' && index!=-1) || rechargePrice.length>=11) return;
          if(index!=-1){
        ...
        onDelete={(val)=>{
          this.setState({
            rechargePrice:rechargePrice?rechargePrice.substring(0,rechargePrice.length-1):''
          },()=>{
            this.handleFontPrice()
          })
        }}
